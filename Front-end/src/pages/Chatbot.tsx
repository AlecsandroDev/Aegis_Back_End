...
import { useChat, MediaAttachment } from "@/contexts/ChatContext"; // Seu ChatContext existente
import { useAuth } from "../contexts/AuthContext"; // <<--- 1. IMPORTAÇÃO DO AuthContext
import {
  generateBotResponse,
  getSpecialContent,
...
const Chatbot = () => {
  const { user, isLoading: authIsLoading, isAuthenticated } = useAuth(); // <<--- 2. USANDO O HOOK useAuth
  const [greeting, setGreeting] = useState("");
  const [question, setQuestion] = useState("");
  const [showOptions, setShowOptions] = useState(false);
  const [userInput, setUserInput] = useState("");
  const [isTyping, setIsTyping] = useState(false); // Typing do bot
  const [insightDialogOpen, setInsightDialogOpen] = useState(false);
  const [emergencyDialogOpen, setEmergencyDialogOpen] = useState(false);
  const [mediaSelectorOpen, setMediaSelectorOpen] = useState(false);
  // const [userName, setUserName] = useState(''); // Removido - usaremos user.first_name do AuthContext
  ...
  // Removido o useEffect que carregava 'userProfile' do localStorage

  // useEffect para montar a saudação (agora depende do 'user' do AuthContext e 'authIsLoading')
  useEffect(() => {
    if (authIsLoading) {
      setGreeting("Carregando..."); // Ou outra mensagem de carregamento
      return;
    }
    ...
    // Usa user.first_name do contexto, se disponível e autenticado
    const displayName =
      isAuthenticated && user?.first_name ? `, ${user.first_name}` : "";
    const fullGreeting = `${timeGreeting}${displayName}`;
    setGreeting(fullGreeting);
  }, [user, isAuthenticated, authIsLoading]); // <<--- 3. DEPENDÊNCIAS ATUALIZADAS

  // useEffect para digitar a pergunta (mantido, mas depende de 'greeting' que agora é dinâmico)
  useEffect(() => {
    if (!greeting || greeting === "Carregando...") return; // Não inicia se a saudação não estiver pronta

    const fullQuestion =
      "Como posso ajudar você com sua segurança digital hoje?";
    setQuestion(""); // Reseta a pergunta para redigitar
    let currentIndex = 0;
  ...
  const handleSendMessage = () => {
    if (userInput.trim() || selectedMedia) {
      ...
      setTimeout(() => {
        const response = generateBotResponse(userInput); // Sua lógica de resposta do bot
  ...
  const confirmEmergencyAction = () => {
    ...
    const response = handleEmergencyContact(); // Sua lógica de contato de emergência
    addMessage(response.text, response.type);
  ...
  const handleSpecialAction = (type: "insight" | "tip") => {
    ...
    if (type === "tip") {
      addMessage(
        "Você gostaria de acessar as dicas úteis de segurança?",
        "bot"
      );
      navigate("/aegis-team"); // Navega para a página da equipe/dicas
      return;
    }

    const content = getSpecialContent(type); // Sua lógica para conteúdo especial
  ...
  // Se o AuthContext ainda estiver carregando as informações de autenticação
  if (authIsLoading) {
    return (
      <div className="min-h-screen w-full bg-gradient-to-br from-aegis-darker via-black to-aegis-dark flex flex-col items-center justify-center text-white">
        Carregando chatbot seguro...
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-aegis-darker via-black to-aegis-dark flex flex-col relative overflow-hidden">
      {/* ... (Seu JSX para MediaSelector, AlertDialogs, background, header) ... */}
      {mediaSelectorOpen && (
        <MediaSelector
      ...
      >
        ...
        <AlertDialogContent className="bg-gray-900 border-red-500/30">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-gray-300">
              Esta opção deve ser usada apenas em situações de emergência. Um
              especialista da equipe Aegis será notificado imediatamente.
              <br />
              <br />
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div
          className="absolute bottom-0 right-1/4 w-[400px] h-[400px] rounded-full bg-aegis-purple/10 blur-[100px] animate-pulse"
      ...
      <div className="flex-1 flex flex-col items-center justify-between p-6 z-10 max-w-md mx-auto w-full">
        <div className="w-full mt-6 text-center">
          {/* A saudação agora usa o nome do usuário do AuthContext */}
          <h1 className="text-2xl font-medium text-white">{greeting}</h1>
          <p className="text-aegis-purple/70 mt-1">
            Aegis Security ao seu dispor
          </p>
        </div>

        {/* Área do Chat (mantida como estava) */}
              ...
              <div className="flex flex-col space-y-4 pb-4">
                ...
                {isTyping && (
                  <div className="flex items-center space-x-2 text-white/50">
                    ...
                    <div
                      className="w-2 h-2 rounded-full bg-aegis-purple/50 animate-pulse"
        ...
        {/* Controles de Input e Botões (mantidos como estavam) */}
        <div
          ...
          <div className="relative w-full">
            ...
            {selectedMedia && (
              <div className="absolute left-4 bottom-16 bg-aegis-purple/20 text-white text-xs py-1 px-3 rounded-full">
                ...
                {selectedMedia.name.length > 20
                  ? selectedMedia.name.substring(0, 20) + "..."
                  : selectedMedia.name}{" "}
                <button
            ...
            <Button
              ...
              className={`absolute right-2 top-1/2 -translate-y-1/2 h-10 w-10 p-0 rounded-full ${
                isTyping
                  ? "bg-gray-500/50"
                  : "bg-gradient-to-br from-aegis-purple to-purple-500 hover:opacity-90"
              }`}
